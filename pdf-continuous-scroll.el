  ...
  (cond ((equal pdf-view-continuous 'hack)
         (let ((hscroll (window-hscroll))
           ...
           (when (= (window-vscroll nil pdf-view-have-image-mode-pixel-vscroll)
                    (image-next-line arg))
             (cond ((if (equal (frame-parameter nil 'fullscreen) 'fullboth)
...
at the top edge of the page moves to the previous page."
  ...
  (cond ((equal pdf-view-continuous 'hack)
         (let ((hscroll (window-hscroll))
           ...
           (when (= (print (window-vscroll nil pdf-view-have-image-mode-pixel-vscroll) (get-buffer-create "out"))
                    (print (image-previous-line arg) (get-buffer-create "out")))
             ...
             (image-set-window-hscroll hscroll)
             (window-resize (get-buffer-window) 1 nil t))
           (if (< (window-total-height) (- (frame-height) window-min-height))
               (condition-case nil
                   (window-resize (get-buffer-window) 1 nil t)
                 (error nil))
             (windmove-down)
